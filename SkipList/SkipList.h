...
// class to manage skiplist

class SkipList
{
private:
    int levelSize;      // keep track of level heighth
    Node* head;         // head node
    Node* bottom;       // used to point to the bottom level for quicker traversal - ended up not using
    Node* arr[];        // used to point to each head node on each lvl
public:
    // SKIPLIST.cpp
    ...
    SkipList() : levelSize(0), head(nullptr), bottom(nullptr) {}
    ~SkipList() {}
    SkipList(int sizeValue, Node* headValue, Node* bottomValue) : levelSize(sizeValue), head(headValue), bottom(bottomValue) {}
    int getLevelSize() { return levelSize; }
    Node* getHead() const { return head; }
    Node* getBottom() const { return bottom; }
    void setLevelSize(int sizeValue) { levelSize = sizeValue; }
    void setHead(Node* headValue) { head = headValue; }
    void setBottom(Node* bottomValue) {bottom = bottomValue; }
    friend ostream& operator << (ostream& output, SkipList& obj) { return output << obj.levelSize; }
    ...
    void printLevels()
    {
        ...
        // iterate each level in sort of a double loop method
        for (int i = levelSize; i > 0; i--)
        {
            ...
            while (iterArr[i] != nullptr)
            {
                ...
                if(iterArr[i]->getData() > -numeric_limits<int>::infinity())
...
typedef SkipList* SkipPtr; // typedef pointer to communicate to skiplist class
